 ...
 *
 * <p>Call {@link #threadWait(long)} or {@link #sleep(long)} to from the main unit test
 * thread to wait for some other thread to perform assertions. These operations will block until
 * {@link #resume()}, the operation times out, or a threadAssert call fails.
 *
 * <p>The threadAssert methods can be used from any thread to perform concurrent assertions.
...
public abstract class ConcurrentTestCase {
   ...
   *
   * <p>Note: This method is likely not very useful since a concurrent run of a test case resulting
   * in the need to resume from a separate thread would yield no correlation between the initiating
   ...
   *
   * <p>Call {@link #resume()} to interrupt the sleep.
   *
   * <p>Note: A sleep time of 0 will sleep indefinitely. This is only recommended to use if you are
   ...
   */
  protected void threadWait(long waitTime, int resumeCount)
    throws Throwable {
   ...
   *
   * <p>Call {@link #finish()} to interrupt the wait.
   *
   * <p>Note: A wait time of 0 will wait indefinitely. This is only recommended to use if you are
